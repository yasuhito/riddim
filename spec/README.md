...
リポジトリのルートから、**3つの正常なモデルの検査だけ**を実行する:

```sh
bundle exec rake model
```

それぞれPlusCalの変換とTLCの検査を順に行う。正常時は各モデルの`名前: OK`だけを表示し、変換や検査に失敗したときは詳細とTLCの反例を表示して失敗する。TLCの警告も表示する。失敗例の作成・検査は含めない。このタスクはJavaが必要なため、通常の`rake`やCIには含めない。変換器は`.tla`内の`BEGIN TRANSLATION`以降を生成し直すので、PlusCalを編集した場合は生成差分も確認する。`spec/*.old`のバックアップはGitから除外している。
...
## pane消滅の確認なしに札を消さない: RetainRecordUntilGone

Riddimの**起動失敗後の片付けだけ**を扱う。自分の所有recordがある状態から、exact paneのcloseが成功するか失敗するかを選び、成功した場合だけ`pane get`の結果を`gone`、`present`、`unknown`から選ぶ。`gone`は対象paneの構造化された`pane_not_found`応答を意味する。**close成功と`gone`の両方が確認できない限りrecordを残す**ことをTLCで検査する（11個の到達状態）。状態の組み合わせの検査であり、Herdr通信やJSONの解釈自体をモデル化していない。

一時コピーだけで`RemoveOwnRecord`の`panePresence = "gone"`条件を外すと、close成功後にpaneが`present`でもrecordを消す反例が出る。本体には変更を残していない。Riddimでは`lib/riddim/herdr/cleanup.rb`のexact pane probeが消滅を判定し、`lib/riddim/start.rb`が世代確認付きの削除を呼ぶ。fake Herdrの`features/start.feature`では、close失敗、paneが残る場合、読み取りが曖昧な場合をそれぞれCLIで確かめる。Firstmateの`bin/fm-teardown.sh`も正確なpaneの消滅を確認できない限り永続recordを残す。Riddimはより狭く、closeそのものが成功した場合だけ`pane get`で消滅を確認し、teardown全体は実装していない。
